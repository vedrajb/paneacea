...
fn extracts_osc_title_and_preserves_terminal_bytes() {
    ...
    let (output, replies, title, cwd) = queries.process_with_title(data, &mut parser);
...
fn extracts_title_when_st_terminator_is_fragmented() {
    ...
    let (_, _, title, cwd) = queries.process_with_title(b"\x1b]2;node", &mut parser);
    assert!(title.is_none());
    assert!(cwd.is_none());

    let (_, _, title, cwd) = queries.process_with_title(b"\x1b\\", &mut parser);
